...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    dict_params = {'W1':  np.random.normal(size =(input_size, num_hidden)),
...
def forward_prop(data, labels, params):
    ...
    """
    # *** START CODE HERE ***
    a = sigmoid(data.dot(params['W1']) +params['b1'] ) #(nexp, num_hidden) ( 60000,300 )
    z = a.dot(params['W2']) + params['b2'] #(nexp, num_output)( 60000,10)
    yhat = softmax(z) #(nexp, num_output)( 60000,10)
    average_loss = np.sum(-labels*np.log(yhat)) / yhat.shape[0] #average cross entropy loss
    return a, yhat, average_loss 
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    a, yhat, average_loss  = forward_prop_func(data,labels,params )
    b = len(yhat)
    dW2 = a.T.dot(yhat - labels) / b
    db2= np.sum(yhat - labels, axis = 0)/b
    p1 = (yhat-labels).dot(params['W2'].T) * a * (1-a)
    dW1 = data.T.dot(p1)/b
    db1 = np.sum((yhat-labels).dot(params['W2'].T) * a * (1-a),axis=0) / b
    
    gradients = {'W2': dW2, 'b2': db2, 'W1': dW1, 'b1': db1}
    return gradients
    # *** END CODE HERE ***
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    a, yhat, average_loss  = forward_prop_func(data,labels,params )
    b = len(yhat)
    dW2 = reg *2*params['W2'] + a.T.dot(yhat - labels) / b
    db2= np.sum(yhat - labels, axis = 0)/b
    p1 = (yhat-labels).dot(params['W2'].T) * a * (1-a)
    dW1 =  reg *2* params['W1'] + data.T.dot(p1)/b
    db1 = np.sum((yhat-labels).dot(params['W2'].T) * a * (1-a),axis=0) / b
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for batch in range(0, len(train_data),batch_size):
        gradients=  backward_prop_func(train_data[batch:batch+batch_size,:], 
