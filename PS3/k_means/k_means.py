...
def init_centroids(num_clusters, image):
    ...
    #set each cluster centroid to the (r, g, b)-values 
    #of a randomly chosen pixel in the image
    
    centroids_init = image[np.random.randint(128, size=16), np.random.randint(128, size=16), :]
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    sum_centroid = np.zeros(shape=[num_clusters])
    sum_x = np.zeros(shape=[num_clusters, C])
    while i < max_iter:
        i += 1
        obs = np.reshape(image, (-1, image.shape[-1]))
        for j in range(len(obs)):
            centroids_index = np.argmin(np.linalg.norm(obs[j] - centroids,  ord=2, axis=1))
            sum_x[centroids_index] += obs[j]
            sum_centroid[centroids_index] += 1 
        
        
    new_centroids = (sum_x.T /sum_centroid).T
    # *** END YOUR CODE ***
...
def update_image(image, centroids):
    ...
    for x in range(H):
        for y in range(W):
            centroids_index = np.argmin(np.linalg.norm(image[x,y,:] - centroids,  ord=2, axis=1))
            image[x, y, :] = centroids[centroids_index ]
    # *** END YOUR CODE ***
