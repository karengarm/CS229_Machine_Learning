...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        # (2) M-step: Update the model parameters phi, mu, and sigma
        phi, mu, sigma = M_step_semi_supervised(x, x_tilde, z_tilde, w, phi, mu, sigma, alpha)
...
def E_step(x, w, phi, mu, sigma):
    """
    E_step: Update your estimates in w
    """
    n, d = x.shape
    for i in range(n):
        for g in range(K):
            w[i, g] = probability(x[i], mu[g], sigma[g]) * phi[g]
...
def probability(x, mu, sigma):
    ...
    w = c * np.exp(-.5 * x_minus_mu.dot(sigma_inv).dot(x_minus_mu.T))
    return w
...
def M_step_semi_supervised(x, x_tilde, z_tilde, w, phi, mu, sigma, alpha):
    n, d = x.shape
    n_tilde, d = x_tilde.shape
...
def log_likelihood(x, phi, mu, sigma, z= None):
    ...
    for i in range(n):
        if z is None:  
            p_x = 0.
            for j in range(K):
                p_x += probability(x[i], mu[j], sigma[j]) * phi[j]
        else:  
            j = int(z[i])
            p_x = probability(x[i], mu[j], sigma[j]) * phi[j]
        ll += np.log(p_x)
