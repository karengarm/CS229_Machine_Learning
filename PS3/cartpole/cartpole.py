...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    policy0 = mdp_data['transition_probs'][state, :, 0].dot(mdp_data['value'])
    policy1 = mdp_data['transition_probs'][state, :, 1].dot(mdp_data['value'])
    
    if policy0 > policy1:
        action = 0
    elif policy1 > policy0:
        action = 1
    else:
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    (since rewards are -1 or 0, you just need to 
     record number of times reward -1 is seen in 'reward_counts' index new_state,0)
    ...
    mdp_data['reward_counts'][new_state, 0] +=  reward #number of times reward -1 is seen
    mdp_data['reward_counts'][new_state, 1] += 1 # number of time `new_state` was reached
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for s in range(mdp_data['num_states']):
        times_action0_in_s = np.sum(mdp_data['transition_counts'][s, :,0])
        times_action1_in_s = np.sum(mdp_data['transition_counts'][s, :,1])
        if  times_action0_in_s > 0:
            mdp_data['transition_probs'][s,:,0] = mdp_data['transition_counts'][s, :,0]/  times_action0_in_s
        if  times_action1_in_s > 0:
            mdp_data['transition_probs'][s, :,1] = mdp_data['transition_counts'][s, :,1]/  times_action1_in_s
            

    mask = (mdp_data['reward_counts'][:, 1] > 0)  # number of time `new_state` was reached
    mdp_data['reward'][mask] = mdp_data['reward_counts'][mask, 0] / mdp_data['reward_counts'][mask, 1]
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        iterations += 1 
        value0 =  mdp_data['transition_probs'][:, :, 0].dot(mdp_data['value']) # (163, )
        value1 =  mdp_data['transition_probs'][:, :, 1].dot(mdp_data['value']) # (163, )
        policy = np.maximum(value0, value1)
        
        old_value =  mdp_data['value']
        mdp_data['value'] = mdp_data['reward'] + gamma *  policy
        
        maximum_absolute_change  = np.max(abs(mdp_data['value'] - old_value))
   
        if maximum_absolute_change < tolerance:
            break
...
def main(plot=True):
    ...
    if plot:
        ...
        plt.savefig('./control_randomseed'+ str(random_seed)+'.png')
